...
gen     fips2 = 1  if state == "CA" 
replace fips2 = 2  if state == "WA" | state == "OR" | state == "AK" | state == "HI"
replace fips2 = 3  if state == "ID" | state == "MT" | state == "WY" | ///
					  state == "UT" | state == "CO" 
replace fips2 = 4  if state == "AZ" | state == "NM" | state == "NV" 
replace fips2 = 5  if state == "OK" | state == "AR" | state == "LA" 
replace fips2 = 6  if state == "TX"
replace fips2 = 7  if state == "ND" | state == "SD" | state == "MN" | state == "IA" | ///
					  state == "NE" | state == "KS" | state == "MO"
replace fips2 = 8  if state == "WI" | state == "IN" |  | state == "IL"  
replace fips2 = 9  if state == "MI" | state == "OH"
replace fips2 = 10 if state == "KY" | state == "TN" | state == "MS" | state == "AL" 
replace fips2 = 11 if state == "GA" | state == "FL"
replace fips2 = 12 if state == "WV" | state == "DE" | state == "MD" | state == "VA" | state == "NC"  | state == "SC"
replace fips2 = 13 if state == "PA" | state == "NJ" 
replace fips2 = 14 if state == "NY"
replace fips2 = 15 if state == "CT" | state == "RI" | state == "MA" | state == "VT" | ///
					  state == "NH" | state == "ME"

replace census = "CA" if fips2 == 1
replace census = "Pacific" if fips2 == 2
replace census = "Mountain North" if fips2 == 3
replace census = "Mountain South" if fips2 == 4
replace census = "West South Central" if fips2 == 5
replace census = "TX" if  fips2 == 6
replace census = "West North Central" if fips2 == 7
replace census = "East North Central" if fips2 == 8
replace census = "Rust Belt" if fips2 == 9
replace census = "East South Central" if fips2 == 10
replace census = "South Atlantic" if fips2 == 11
replace census = "Middle Atlantic" if fips2 == 12
replace census = "North Atlantic" if fips2 == 13
replace census = "NY" if fips2 == 14
replace census = "New England" if fips2 == 15
					  
drop fips 
rename fips2 fips


collapse (sum) Lmilitary Lmilitary_all military_all gdp  (mean) bea cpi news (first) census region, by(fips year)
