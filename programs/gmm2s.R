gmm2S <- function(Ti, N, y, X, Z, yy, k_t,tol, guess = -99, maxiter, second) {  #tol, guess, maxiter optional
  ...
  for (k in -yy:yy) { # yy to 0
    ...
    if (second == 0) {
      Omega_hat <- (1/Ti)*w*(t(mom_re[start:finish,])%*%mom_re[(start+k):(finish+k),]) + Omega_hat 
    } else { 
      Omega_U <-(1/Ti)*t(bigU[start:finish,])%*%bigU[(start+k):(finish+k),]
      Omega_Z <- (1/Ti)*t(z_re[start:finish,])%*%z_re[(start+k):(finish+k),]
      Omega_hat <- w*(hadamard.prod(Omega_U,Omega_Z)) + Omega_hat
    }
  }
