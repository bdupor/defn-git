...
bm_agg <- gmm2s_iter(Ti,1,y,X,Z, yy, k_t,tol, guess, maxiter,second)

#Iris:  here is y
y <- matrix(hall$F4Drgdp,Ti*N,1)
X <- model.matrix(~factor(fips) + F4Drbea_alt_nat,data=hall)
Z <- model.matrix(~F4Drbea_alt_nat,data=hall)
bm_aa <- gmm2s_iter(Ti,N,y,X,Z, yy, k_t,tol, guess, maxiter,second)

#Iris:  Hereare are the local and spillover (ie. leaveout) variables in X
X <- model.matrix(~factor(fips) + F4Drbea_alt + F4Drbea_alt_leaveout_sc,data=hall)
Z <- model.matrix(~F4Drbea_alt + F4Drbea_alt_leaveout,data=hall)
bm_lsd <- gmm2s_iter(Ti,N,y,X,Z, yy, k_t,tol, guess, maxiter,second)

#Z <- model.matrix(~F4Drbea_inst + F4Drbea_leaveout_inst,data=hall)
#iv_lsd <- gmm2s_iter(Ti,N,y,X,Z, yy, k_t,tol, guess, maxiter,second)
...
ggsave("../output/bmark_scatterplot_divisionfacet_spill.pdf", bmark_scatterplot_facet, device = "pdf", height = 6, width = 9)
...
bmark_scatterplot_facet <- ggplot(data = hall, aes(x=x, y= y)) +
  ...
  facet_wrap(~census) +
     stat_poly_eq(formula = myformula, 
                ...
                parse = TRUE)
bmark_scatterplot_facet
ggsave("../output/bmark_scatterplot_divisionfacet_own.pdf", bmark_scatterplot_facet, device = "pdf", height = 6, width = 9)
