...
hall <- subset(df2,year>=1961 & year<=2005)
...
Ti <- length(hall$Fhy_inc)/N

y <- matrix(hall$Fhy_inc,Ti*N,1)
X <- model.matrix(~factor(fips) + Fhx2_nat50_inc,data=hall)
Z <- model.matrix(~Fhx2_nat50_inc,data=hall)
...
est_aa <- gmm2S(Ti,N,y,X,Z, yy, k_t,tol, guess, maxiter,second)

X <- model.matrix(~factor(fips) + Fhx2_inc + Fhx2_inc_spill,data=hall)
Z <- model.matrix(~ Fhx2_inc + Fhx2_inc_spill,data=hall)

est_lsd <- gmm2S(Ti,N,y,X,Z, yy, k_t,tol, guess, maxiter,second)

lm_aa <- lm(Fhy_inc~factor(fips)+ Fhx2_nat50_inc,data=hall)

lm_lsd <- lm(Fhy_inc~factor(fips) + Fhx2_inc + Fhx2_inc_spill,data=hall)

lm_loc <- lm(Fhy_inc~factor(fips) + Fhx2_inc,data=hall)
...
bmark_scatterplot_facet <- ggplot(data = hall, aes(x=x, y= y)) +
  ...
  geom_smooth(data = hall, method = "lm", se = FALSE, color = "black", aes(x=Fhx2_spill, y= resid)) + theme_pubr() +
  labs(x='Spillover', y='Residuals') +
  scale_colour_gradient2(low = "red", high = "green", mid = "black") + 
  scale_shape_manual("Division", values = c(0,1,2,3,4,5,6,7,8)) +
  facet_wrap(~fips) +
