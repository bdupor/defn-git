...
yy <- 12        #Bartlett window goes from -yy to yy
second <- 0      #Second moment independence if value = 1

df <- read_dta("./Census_Regions_1953-1974_localFE.dta")




N <- length(unique(df$cenrgn_num))
hall <-df
hall <- hall %>%
  group_by(timeyq) %>%
  mutate(y_nat = sum(y),
...
hallAgg <- subset(hall,cenrgn_num==1)

y <- matrix(hallAgg$y_nat,Ti,1)
X <- model.matrix(~x_nat,data=hallAgg)
Z <- model.matrix(~x_nat,data=hallAgg)
bm_agg <- gmm2S(Ti,1,y,X,Z, yy, k_t,tol, guess, maxiter,second)


y <- matrix(hall$y,Ti*N,1)
X <- model.matrix(~factor(cenrgn_num) + x_nat_sc,data=hall)
Z <- model.matrix(~x_nat_sc,data=hall)
bm_aa <- gmm2S(Ti,N,y,X,Z, yy, k_t,tol, guess, maxiter,second)

y <- matrix(hall$y,Ti*N,1)
X <- model.matrix(~factor(cenrgn_num) + x + xs,data=hall)
Z <- model.matrix(~ x + xs,data=hall)
...
table1[c("agg","agg (se)","aa","aa (se)"),"aggregate"] <-
  c(bm_agg$phi_hat["x_nat",1],
    sqrt(bm_agg$Var["x_nat","x_nat"]),
    bm_aa$phi_hat["x_nat_sc",1],
    sqrt(bm_aa$Var["x_nat_sc","x_nat_sc"]))

table1[c("aa","lsd"),"pval"] <-
  c(bm_aa$pval,bm_lsd$pval)

table1[c("lsd","lsd (se)"),"local"] <-
  c(bm_lsd$phi_hat["x",1],
    sqrt(bm_lsd$Var["x","x"]))

table1[c("lsd","lsd (se)"),"spillover"] <-
  c(bm_lsd$phi_hat["xs",1],
    sqrt(bm_lsd$Var["xs","xs"]))

temp <- c("x","xs")
h <- matrix(c(1,1),1,2)

table1[c("lsd","lsd (se)"),"aggregate"] <-
