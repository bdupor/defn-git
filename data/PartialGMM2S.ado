...
mata{
		...
		*/
		z2 = J(N*T,pN*K,0)
		for (p=1; p<=pN; p++){
			* P_r is a scalar for each unique partition (first column of unique_part)
			p_r = unique_part[p,1]
			* for all of the divisions
			for (i = 1; i<=N; i++){
				* create scalars ii and jj (what are they???)
				ii = T*(i-1) +1
				jj = K*(p-1) +1
				* if at a certain partition?
				if (p_r == part[i,1]){
					* sum over the division 
					aux = sum(part:==part[i,1])
					* then let z2 matrix at index be equal to the instrument matrix 
					* divided by the sum over the divisions
...
mata:
	...
	function m_t(t,u,z,N,T,part){
		...
		* scalar for indexed division
			p_r = part[i,1]
			
			* for all of the divisions 
			* what is difference between N and pN?
			for (r=1; r<=pN; r++){
				* if division is equal to indexed division
				if (r == p_r){
					* sum over the partition
					aux = sum(p_r:==part)
					
					* Set matrix P_aux to be 1/that sum 
					P_aux[r,i] = 1  / aux
				}
			}
		}
		...
		* For all divisions
		for (i = 1; i<=N; i++) {
			* For all instruments? 
			for (r = 1; r<=K; r++){
				* Multiply the number of instruments by the division -1 and add the division? Huh?
				idx = K*(i - 1) + r 
				* Multiply the time by the time - 1
				j = t+T*(i-1)
				* Set the matrix at the first scalar to be u*z 
				m[idx,1] =  u[j,1]*z[j,r]  
			}
		}
